...
import com.linkid.livestreaming.api.LinkIDUIKitPrebuiltLiveStreamingService;
import com.linkid.livestreaming.api.common.Common;
import com.linkid.livestreaming.api.common.RoleChangedListener;
import com.linkid.livestreaming.api.pk.PK;
...
import com.linkid.livestreaming.internal.core.PKService;
...
import com.linkid.livestreaming.internal.core.LinkIDLiveStreamingPKBattleRejectCode;
...
public class LinkIDLiveStreamingManager {

    private LinkIDLiveStreamingManager() {
    }

    private static final class Holder {

        private static final LinkIDLiveStreamingManager INSTANCE = new LinkIDLiveStreamingManager();
    }

    public static LinkIDLiveStreamingManager getInstance() {
        return LinkIDLiveStreamingManager.Holder.INSTANCE;
    }
    ...
    private CopyOnWriteArrayList<LinkIDLiveStreamingListener> liveStreamingListenerList = new CopyOnWriteArrayList<>();
    private CopyOnWriteArrayList<RoleChangedListener> roleChangedListeners = new CopyOnWriteArrayList<>();
    private LinkIDLiveStreamingRole currentRole;
    ...
    private LinkIDUIKitPrebuiltLiveStreamingConfig liveConfig;


    void init(Application application, Long appID, String appSign) {
        ZegoUIKit.init(application, appID, appSign, ZegoScenario.GENERAL);
        if (liveConfig.bottomMenuBarConfig.hostButtons.contains(LinkIDMenuBarButtonName.BEAUTY_BUTTON)
            || liveConfig.bottomMenuBarConfig.coHostButtons.contains(LinkIDMenuBarButtonName.BEAUTY_BUTTON)
            || liveConfig.bottomMenuBarConfig.audienceButtons.contains(LinkIDMenuBarButtonName.BEAUTY_BUTTON)) {
                    ...
                    if (coHostInvitations.contains(invitationID)) {
                        if (Objects.equals(getHostID(), invitee.userID)) {
                            ...
                            if (uiCallBack != null) {
                                ...
                                LinkIDUIKitPrebuiltLiveStreamingService.common.showTopTips(string, false);
                            }
                            if (uiCallBack != null) {
                                uiCallBack.showRequestCoHostButton();
                            }
    ...
    public LinkIDTranslationText getTranslationText() {
    ...
    public void endCoHost() {
        ...
        setCurrentRole(LinkIDLiveStreamingRole.AUDIENCE);
    }

    public void setCurrentRole(LinkIDLiveStreamingRole liveStreamingRole) {
        ...
        if (notifyChange) {
            ...
            for (LinkIDLiveStreamingListener listener : liveStreamingListenerList) {
                listener.onRoleChanged(currentRole);
            }
        }
    }

    public LinkIDLiveStreamingRole getCurrentUserRole() {
        return currentRole;
    }
    ...
    public void setPrebuiltConfig(LinkIDUIKitPrebuiltLiveStreamingConfig config) {
    ...
    public LinkIDUIKitPrebuiltLiveStreamingConfig getPrebuiltConfig() {
        return liveConfig;
    }
    ...
    public void rejectPKBattleStartRequest(String requestID) {
        pkService.rejectPKBattleStartRequest(requestID, LinkIDLiveStreamingPKBattleRejectCode.HOST_REJECT.ordinal());
    }
     ...
     * {@link com.linkid.livestreaming.api.pk.Events#addPKListener(PKListener)}  instead.
     * <p>
     * <p>
     * if listen to roleChanged events, use
     * {@link com.linkid.livestreaming.api.common.Events#addRoleChangedListener(RoleChangedListener)}
     * instead
     */
    @Deprecated
    public void addLiveStreamingListener(LinkIDLiveStreamingListener listener) {
     ...
     */
    @Deprecated
    public void removeLiveStreamingListener(LinkIDLiveStreamingListener listener) {
        removePKListener(listener);
        liveStreamingListenerList.remove(listener);
    }


    public interface LinkIDLiveStreamingListener extends PKListener {

        default void onRoleChanged(LinkIDLiveStreamingRole liveStreamingRole) {
        }
    }
}
