...
public class PKService {
    ...
    public void onInvitationReceived(ZegoUIKitUser inviter, int type, String data) {
        ...
        if (pkExtendedData != null && invitationID != null) {
            if (pkExtendedData.type == PKExtendedData.START_PK) {
                ...
                boolean userNotHost = TextUtils.isEmpty(currentRoomID) || (!LinkIDLiveStreamingManager.getInstance().isCurrentUserHost());
                boolean alreadySend = sendPKStartRequest != null;
                boolean alreadyReceived = recvPKStartRequest != null;
                boolean liveNotStarted = !LinkIDLiveStreamingManager.getInstance().isLiveStarted();
                boolean isInAPK = currentPKInfo != null;
                if (userNotHost || isInAPK || alreadySend || alreadyReceived || liveNotStarted) {
                    LinkIDLiveStreamingPKBattleRejectCode rejectCode;
                    if (userNotHost) {
                        rejectCode = LinkIDLiveStreamingPKBattleRejectCode.USER_NOT_HOST;
                    } else if (isInAPK) {
                        rejectCode = LinkIDLiveStreamingPKBattleRejectCode.IN_PK;
                    } else if (alreadySend) {
                        rejectCode = LinkIDLiveStreamingPKBattleRejectCode.ALREADY_SEND;
                    } else if (alreadyReceived) {
                        rejectCode = LinkIDLiveStreamingPKBattleRejectCode.ALREADY_RECEIVED;
                    } else {
                        rejectCode = LinkIDLiveStreamingPKBattleRejectCode.LIVE_NOT_STARTED;
                    }
    ...
    private void onReceivePKRoomAttribute(Map<String, String> roomProperties) {
        ...
        if (LinkIDLiveStreamingManager.getInstance().isCurrentUserHost()) {
            ...
            if (currentPKInfo == null) {
                ...
                if (LinkIDLiveStreamingManager.getInstance().getHostID() != null) {
                    setCurrentPKInfo(pkInfo);
                    startPKBattle();
                }
            }
    ...
    private void startPKBattle() {
        if (LinkIDLiveStreamingManager.getInstance().isCurrentUserHost()) {
            LinkIDLiveStreamingManager.getInstance().startPublishingStream();
            startMixStreamTask(false, new IZegoMixerStartCallback() {
                @Override
                public void onMixerStartResult(int errorCode, JSONObject extendedData) {
                    if (errorCode == 0 && getPKInfo() != null) {
    ...
    public void stopPKBattle() {
        if (LinkIDLiveStreamingManager.getInstance().isCurrentUserHost()) {
            ...
            if (!ZegoUIKit.isCameraOn(localUser.userID) && !ZegoUIKit.isMicrophoneOn(localUser.userID)) {
                LinkIDLiveStreamingManager.getInstance().stopPublishStream();
            }
            deletePKRoomAttributes();
            stopMixTask();
    ...
    public void muteHostAudioVideo(boolean mute) {
        ...
        if (hostUser != null) {
            String currentRoomID = ZegoUIKit.getRoom().roomID;
            String streamID = LinkIDLiveStreamingManager.generateCameraStreamID(currentRoomID, hostID);
    ...
    private void startMixStreamTask(boolean muteAudio, IZegoMixerStartCallback callback) {
        ...
        String streamID = LinkIDLiveStreamingManager.generateCameraStreamID(currentRoomID, localUser.userID);
    ...
    public static class PKInfo {
        ...
        public String getPKStream() {
            return LinkIDLiveStreamingManager.generateCameraStreamID(pkRoom, pkUser.userID);
        }
